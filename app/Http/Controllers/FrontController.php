...
class FrontController extends Controller
{
    ...
    /* add to cart & order place */
        ...
        public function checkout(Request $request){
            ...
            $data['deviceId']               = $deviceId;
            $data['cartItems']              = OrderDetail::where('cust_device_id', '=', $deviceId)->where('is_cart', '=', 1)->where('status', '=', 0)->get();
            $data['countries']              = Country::select('name')->where('status', '=', 1)->get();
            $uId                            = session('user_id');
                    ...
                    if($checkEmail <= 0){
                        ...
                        if($checkPhone <= 0){
                            if($request->password == $request->confirm_password){
                                ...
                                return redirect(url('signup-validate-otp/'.Helper::encoded($id)))->with('success_message', 'Please Verify Your Email With OTP Send To Your Inbox !!!');
                            } else {
                                return redirect()->back()->with('error_message', 'Password & Confirm Password Mismatched !!!');
                            }
                        } else {
                            return redirect()->back()->with('error_message', 'Phone Number Already Exists. Try With Different Phone Number !!!');
                        }
                    } else {
            ...
            if($request->isMethod('post')){
                ...
                if($postData['mode'] = 'profile'){
                    ...
                    if($this->validate($request, $rules)){
                        ...
                        if($redirectLink == ''){
                            return redirect()->back()->with('success_message', $type.' Details Inserted Successfully !!!');
                        } else {
                            return redirect($redirectLink)->with('success_message', $type.' Details Inserted Successfully !!!');
                        }
                    } else {
                        return redirect()->back()->with('error_message', 'All Fields Required !!!');
                    }
