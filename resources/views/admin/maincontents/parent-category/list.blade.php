                ...
                <tr>
                  ...
                  <!-- <td>
                    <?php if($row->cover_image != ''){?>
                      <img src="<?=env('UPLOADS_URL').'category/'.$row->cover_image?>" alt="<?=$row->category_name?>" style="width: 100px; height: 100px; margin-top: 10px;border-radius: 50%;">
                    <?php } else {?>
                      <img src="<?=env('NO_IMAGE')?>" alt="<?=$row->category_name?>" class="img-thumbnail" style="width: 100px; height: 100px; margin-top: 10px;">
                    <?php }?>
                  </td> -->
                  <td>
                    <?php
                    $getProductCount = Product::where('status', '!=', 3)->where('main_category', '=', $row->id)->count();
                    ?>
                    <a href="<?=url('admin/parent-category/get-main-category-products/' . Helper::encoded($row->id))?>" target="_blank" class="badge bg-info"><?=$getProductCount?></a>
                  </td>
                  <td>
