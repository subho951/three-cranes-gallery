            ...
            <tbody>
              <?php if($rows){ $sl=1; foreach($rows as $row){?>
                <tr>
                  ...
                  <td>
                    <?php
                    $getProductCount = Product::where('status', '!=', 3)->where('sub_category', '=', $row->id)->count();
                    ?>
                    <a href="<?=url('admin/sub-category/get-sub-category-products/' . Helper::encoded($row->id))?>" target="_blank" class="badge bg-info"><?=$getProductCount?></a>
                  </td>
                  <td>
